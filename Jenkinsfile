    ...
    stage('SonarQube Scan') {
        steps {
            withSonarQubeEnv('sonarqube') {
                sh 'mvn sonar:sonar -Dsonar.projectKey=employee-service'
            }
        }
    }
    ...
    stage('Update GitOps Repo') {
        steps {
            withCredentials([
                ...
                sed -i 's#image: srihari25/employee-service:.*#image: srihari25/employee-service:${BUILD_NUMBER}#' deployment.yaml
